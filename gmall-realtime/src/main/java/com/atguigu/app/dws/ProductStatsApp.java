    ...
    public static void main(String[] args) throws Exception {
        ...
        */
        ...
        SingleOutputStreamOperator<ProductStats> pageAndDispStatsDstream = pageViewDStream.process(new ProcessFunction<String, ProductStats>() {
            @Override
            public void processElement(String json, ProcessFunction<String, ProductStats>.Context ctx, Collector<ProductStats> out) throws Exception {
                ...
                JSONArray dispalys = jsonObj.getJSONArray("dispaly");
                if (dispalys != null && dispalys.size() > 0) {
                    for (int i = 0; i < dispalys.size(); i++) {
                        JSONObject display = dispalys.getJSONObject(i);
